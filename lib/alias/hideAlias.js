...
module.exports = function (client, message, args) {
    ...
    if (typeof client.guild.config.hidden == "undefined" || client.guild.config.hidden.length == 0) {
    ...
    args.forEach(arg => {
        ...
        for (a in client.guild.config.alias) {
            if (client.guild.config.alias[a].includes(arg) && !client.guild.config.hidden.includes(arg)) {
    ...
    hiddenAliases.forEach(a => {
        m = m + a + (hiddenAliases.indexOf(a) < hiddenAliases.length - 1 ? ', ' : '');
    });
