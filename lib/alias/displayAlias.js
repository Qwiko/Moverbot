module.exports = function(client, message, arg) {
  ...
  for (key in client.guild.config.alias) {
    ...
    //Slice removes channelname, id and position which is the first three elements.
    client.guild.config.alias[key] = client.guild.config.alias[key].slice(2);

    //Filter away hidden aliases
    if (!arg) {
      //Remove position alias
      client.guild.config.alias[key] = client.guild.config.alias[key].slice(1);
