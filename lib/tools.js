  ...
  client.db.collection("config").findOne(
    ...
    function (err, doc) {
      ...
      for (var id in config.gamemove.users) {
        member = guild.members.find((member) => member.id === id);
        if (!member) {
...
exports.moveMembers = function (client, oldChannel, newChannel) {
  ...
  if (typeof oldChannel.user !== "undefined") {
    ...
    oldChannel.setVoiceChannel(newChannel).catch(console.error);
    counter = 1;
  } else {
    //Moving all members of a channel
    oldChannel.members.forEach((member) => {
      member.setVoiceChannel(newChannel).catch((error) => {
        if (error.code == 40032) {
...
exports.log = function (
  ...
  client.db.collection("logs").insert(logNew, function () {});
};
